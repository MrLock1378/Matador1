...
local function pre_process(msg)
	if msg.to.type ~= 'pv' then
		...
		if gpst and not chex and msg.from.id ~= SUDO and not is_sudo(msg) then
			...
			if lang then
				tdcli.sendMessage(msg.to.id, msg.id_, 1, '_گروه به مدت 1 روز شارژ شد. لطفا با سودو برای شارژ بیشتر تماس بگیرید. در غیر اینصورت گروه شما از لیست ربات حذف و ربات گروه را ترک خواهد کرد.   @LokerTeam™_', 1, 'md')
			else
...
if is_sudo(msg) then
   		if matches[1]:lower() == 'add' and not redis:get('ExpireDate:'..msg.to.id) then
			...
			redis:setex('ExpireDate:'..msg.to.id, 180, true)
				...
				if lang then
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '_گروه به مدت 3 دقیقه برای اجرای تنظیمات شارژ میباشد.  💥 @LokerTeam™ :)_', 1, 'md')
				else
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '_Group charged 3 minutes  for settings. 💥 @LokerTeam™ :) _', 1, 'md')
				end
		end
	...
	if msg.to.type == 'channel' or msg.to.type == 'chat' then
		if matches[1] == 'charge' and matches[2] and not matches[3] and is_sudo(msg) then
			if tonumber(matches[2]) > 0 and tonumber(matches[2]) < 1001 then
				...
				if lang then
					tdcli.sendMessage(msg.to.id, msg.id_, 1, ' 💥 @LokerTeam™ :)ربات با موفقیت تنظیم شد\nمدت فعال بودن ربات در گروه به '..matches[2]..' روز دیگر تنظیم شد...', 1, 'md')
					tdcli.sendMessage(SUDO, 0, 1, 'ربات در گروه '..matches[2]..' به مدت `'..msg.to.id..'` روز تمدید گردید. 💥 @LokerTeam™ :)', 1, 'md')
				else
					tdcli.sendMessage(msg.to.id, msg.id_, 1, ' 💥 @LokerTeam™ :)ربات با موفقیت تنظیم شد\nمدت فعال بودن ربات در گروه به '..matches[2]..' روز دیگر تنظیم شد...', 1, 'md')
					tdcli.sendMessage(SUDO, 0, 1, 'ربات در گروه '..matches[2]..' به مدت `'..msg.to.id..'` روز تمدید گردید. 💥 @LokerTeam™ :)', 1, 'md')
				end
		...
		if matches[1]:lower() == 'check' and is_mod(msg) and not matches[2] then
			...
			if expi == -1 then
				if lang then
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '_گروه به صورت نامحدود شارژ میباشد! 💥 @LokerTeam™ :)_', 1, 'md')
				else
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '_Unlimited Charging! 💥 @LokerTeam™ :)_', 1, 'md')
				end
			else
...
1, nil, send_file, '💥 @LokerTeam™ :)', dl_cb, nil)
	...
	if matches[1]:lower() == "sendplug" and matches[2] or matches[1]:lower() == "Sendplug" and matches[2] then
		...
		tdcli.sendDocument(msg.chat_id_, msg.id_,0, 
1, nil, plug, '💥 @LokerTeam™ :)', dl_cb, nil)
    end
  end
